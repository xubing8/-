...
class TranslatorApp(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        layout.addWidget(self.translate_button, 4, 0)
        layout.addWidget(self.import_button, 4, 1)
        layout.addWidget(self.import_word_button, 5, 0)
        layout.addWidget(self.export_word_button, 5, 1)
        layout.addWidget(self.export_button, 6, 0)
        layout.addWidget(self.view_memory_button, 6, 1)
        layout.addWidget(self.clear_memory_button, 7, 0, 1, 2)  # 清空记忆库按钮占据两列
        layout.addWidget(self.output_label, 8, 0)
        layout.addWidget(self.output_text, 9, 0, 1, 2)
    ...
    def split_sentences(self, text, lang):
        ...
        if lang == '中文':
            ...
            try:
                ...
                language_code = nltk_lang_map.get(lang, 'english')
                sentences = sent_tokenize(text, language=language_code)
            except ImportError:
                # 如果未安装nltk，则使用简单的分句
