...
class RateLimitError(Exception):
    pass
# 定义所需的库
required_packages = {'sentence-splitter','ratelimit','tenacity','anthropic', 'python-dotenv'}
...
class TranslationWorker(QtCore.QObject):
    ...
    def __init__(self, source_text, from_lang, to_lang, engine, translator):
        super().__init__()
        self.source_text = source_text
        self.from_lang = from_lang
        self.to_lang = to_lang
        self.engine = engine
        self.translator = translator  # 传入主窗口实例

    def run(self):
        try:
            logging.info(f"翻译请求 - 源语言: {self.from_lang}, 目标语言: {self.to_lang}, 文本: {self.source_text}")
            from tenacity import RetryError

            # 应用术语替换
            modified_text, terms = self.translator.apply_key_terms(self.source_text, self.from_lang)
            logging.debug(f"应用术语替换后文本: {modified_text}, 术语: {terms}")

            # 将文本按句子分割
            sentences = self.translator.split_sentences(modified_text, self.from_lang)
            ...
            for sentence in sentences:
                ...
                # 尝试从句子翻译记忆库中查找相似翻译
                similar_translation, similarity = self.translator.find_similar_sentence(sentence)
                ...
                if similar_translation:
                    ...
                    if self.engine == 'claude':
                        # 使用 Claude API 进行翻译
                        machine_translation = self.translator.claude_translate(sentence, self.from_lang, self.to_lang)
                        logging.info(f"Claude 翻译结果: {machine_translation}")
                    elif self.engine == 'baidu':
                        # 使用百度翻译API进行翻译
                        machine_translation = self.translator.baidu_translate(sentence, self.from_lang, self.to_lang)
                        logging.info(f"Baidu 翻译结果: {machine_translation}")
                    elif self.engine == 'kimi':
                        # 使用 Kimi API 进行翻译
                        machine_translation = self.translator.kimi_translate(sentence, self.from_lang, self.to_lang)
                    ...
                    if machine_translation.startswith('翻译失败') or machine_translation.startswith('请求翻译服务出错'):
                        ...
                        # 进行词汇级别的替换
                        translated_sentence = self.translator.replace_words(machine_translation, self.to_lang, self.from_lang)
                        logging.debug(f"词汇替换后译文: {translated_sentence}")

                        # 添加到翻译记忆库
                        self.translator.add_sentence_translation(sentence, translated_sentence, self.from_lang, self.to_lang)
            ...
            # 恢复术语
            final_translation = self.translator.restore_key_terms(final_translation, terms)
            ...
            self.progress.emit(table_html)

        except RetryError as re:
            logging.error(f"翻译过程中发生重试错误: {re}", exc_info=True)
            self.progress.emit('翻译请求多次失败，可能由于API访问限制。请稍后再试或检查API配额。')
        except Exception as e:
            logging.error(f"翻译过程中发生错误: {e}", exc_info=True)
            self.progress.emit(f'翻译失败：{str(e)}')
        finally:
            self.finished.emit()
...
class TranslatorApp(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        self.engine_select.addItems(['百度翻译API', 'Kimi API'])  # 添加 'Kimi API'
    ...
    def translate_text(self):
        ...
        self.worker = TranslationWorker(source_text, from_lang, to_lang, engine, self)
        ...
        self.worker.progress.connect(self.update_translation_result)
    ...
    def update_translation_result(self, result):
        ...
        self.output_text.insertHtml(result)
        # 重新启用翻译按钮
        self.translate_button.setEnabled(True)

    def apply_key_terms(self, text, from_lang):
        """
        根据术语库替换文本中的关键名词。
        """
        self.cursor.execute("SELECT source_term, target_term FROM key_terms WHERE from_lang=?", (from_lang,))
